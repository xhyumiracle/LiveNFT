...
Hero.prototype.run = function(delta){
...
Hero.prototype.dorun=function(delta){
    hero.run(delta);
    var deltax=Math.sin(this.mesh.rotation.y)
    var deltaz=Math.cos(this.mesh.rotation.y)
    hero.mesh.position.x+=deltax*0.05;
    hero.mesh.position.z+=deltaz*0.05;
}

Hero.prototype.fn=function(time, rect, delta){
    // this.mesh.rotation.y = time * 1;

    // const {left, right, top, bottom, width, height} = rect;
    // console.log('test')
    // console.log(hero.mesh.position.x, hero.mesh.position.z)
  ...
  switch (hero.status){
    case 'running':
      ...
      if (!isOutScene){
        ...
        hero.mesh.rotation.y += Math.random()%(Math.PI)
        hero.dorun(delta)
        hero.dorun(delta)
      }
...
function create(){
  ...
  hero.mesh.position.y=-15;
  hero.mesh.rotation.y = Math.PI/2;
  hero.mesh.scale.set(0.1,0.1,0.1);
  return hero;
}
